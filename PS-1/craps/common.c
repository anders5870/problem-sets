...
void end_handler(int signum)
{
  ...
  if(signum!=SIGUSR2) exit(EXIT_FAILURE);
...
void win_handler(int signum)
{
  ...
  if(signum!=SIGUSR1) exit(EXIT_FAILURE);
...
// shooter - it simulates the players action during a game of luck.
//
...
void shooter(int id, int seed_fd_rd, int score_fd_wr)
{
	...
	// TODO 8: roll the dice, but before that, read a seed from the parent via pipe
        ...
        while(read(seed_fd_rd, &seed, sizeof(int)) < (short)sizeof(int)){}
	...
	// TODO 9: send my score back to the master via pipe
        ...
        while(write(score_fd_wr, &score, sizeof(int)) < (short)sizeof(int)){}
